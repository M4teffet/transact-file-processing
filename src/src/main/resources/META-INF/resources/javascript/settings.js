...
// ── VIREMENT_SALAIRE : comptes de transit par société ───────────────────────
(function () {
    const sfetch = window.secureFetch || ((u, o) => fetch(u, Object.assign({credentials: "same-origin"}, o || {})));

    async function loadVirsalConfig() {
        const listEl = document.getElementById("virsalList");
        if (!listEl) return;
        listEl.innerHTML = '<p class="text-xs" style="color:var(--ink-4)">Chargement…</p>';
        try {
            const res = await sfetch("/api/v1/virsal-config");
            if (!res || !res.ok) throw new Error("load failed");
            const rows = await res.json();
            listEl.innerHTML = "";
            if (!rows.length) {
                listEl.innerHTML = '<p class="text-xs" style="color:var(--ink-4)">Aucune société configurée.</p>';
                return;
            }
            rows.forEach((r) => {
                ...
                label.innerHTML = (r.companyId || "") +
                    ' <span style="color:var(--ink-4)">(' + (r.code || "") + " · " + (r.source || "NONE") + ")</span>";

                const input = document.createElement("input");
                input.className = "form-select";
                input.style.maxWidth = "260px";
                input.placeholder = "compte de transit";
                input.value = r.transitAccount || "";

                const btn = document.createElement("button");
                btn.className = "btn-flux";
                btn.type = "button";
                ...
                btn.addEventListener("click", async () => {
                    if (!input.value.trim()) {
                        msg.textContent = "compte requis";
                        msg.style.color = "#B42318";
                        return;
                    }
                    btn.disabled = true;
                    try {
                        const rr = await sfetch("/api/v1/virsal-config/" + encodeURIComponent(r.companyId), {
                            method: "PUT",
                            headers: {"Content-Type": "application/json"},
                            body: JSON.stringify({transitAccount: input.value.trim()})
                        });
                        if (!rr.ok) throw new Error("save failed");
                        msg.textContent = "✓ enregistré";
                        msg.style.color = "var(--status-success-text)";
                        label.innerHTML = (r.companyId || "") + ' <span style="color:var(--ink-4)">(' + (r.code || "") + " · DB)</span>";
                    } catch (e) {
            ...
            listEl.innerHTML = '<p class="text-xs" style="color:#B42318">Erreur de chargement.</p>';
        }
    }
