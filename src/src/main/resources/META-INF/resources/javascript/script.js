...
async function loadApplications() {
    ...
    try {
        ...
        // Apps reserved for their own dedicated page (e.g. VIREMENT_SALAIRE → "Upload Salaire").
        // They only appear when that page has locked to them; hidden from the generic upload.
        const SALARY_ONLY = ['VIREMENT_SALAIRE'];
        const locked = window.FLUX_LOCKED_APP;
        apps.forEach(app => {
            if (SALARY_ONLY.includes(app.code) && locked !== app.code) return;
            el.add(new Option(`${app.code} – ${app.label}`, app.code));
        });

        // Locked-app mode (dedicated pages like "Upload Salaire"): pre-select the
        // application, hide the picker, and load its fields immediately.
        if (locked) {
            el.value = locked;
            const wrap = el.closest('.app-select-wrap');
            (wrap || el).style.display = 'none';
            if (el.value === locked) loadFields(locked);
        }
    } catch (err) {
...
async function handleUpload() {
    ...
    try {
        ...
        // Let dedicated pages (e.g. Upload Salaire) react — e.g. show the billing selector.
        document.dispatchEvent(new CustomEvent('flux:uploaded', {
            detail: {batchId: data.batchId, application: app}
        }));
