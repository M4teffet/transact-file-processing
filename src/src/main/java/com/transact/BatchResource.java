...
import com.transact.service.VirementSalaireBillingService;
...
public class BatchResource {
    ...
    // ── VIREMENT_SALAIRE : charges décidées par lot après l'upload ───────────────

    /**
     * Aperçu des charges pour un lot VIREMENT_SALAIRE — ne persiste rien.
     * Permet à l'UI d'afficher Z, X, frais et A en direct pendant que l'utilisateur
     * essaie un mode ({@code NONE} / {@code FLAT} / {@code STANDARD}).
     */
    @GET
    @Path("/{id}/billing")
    @RolesAllowed({"INPUTTER", "AUTHORISER", "ADMIN"})
    @Produces(MediaType.APPLICATION_JSON)
    @Operation(summary = "Aperçu des charges d'un lot de virements de salaires (sans persistance)")
    public Response previewBilling(@PathParam("id") String id,
                                   @QueryParam("mode") String mode,
                                   @QueryParam("flatFee") BigDecimal flatFee) {
        String path = "/api/v1/batches/" + id + "/billing";
        ObjectId bId = parseObjectId(id);
        FileBatch batch = FileBatch.findById(bId);
        if (batch == null)
            return Response.status(404).entity(ApiError.of("NOT_FOUND", "Lot non trouvé", path)).build();
        if (!"VIREMENT_SALAIRE".equals(getAppName(batch.applicationId)))
            return Response.status(400).entity(ApiError.of("BAD_REQUEST",
                    "Les charges ne s'appliquent qu'aux lots VIREMENT_SALAIRE", path)).build();

        String resolvedMode = billingService.resolveMode(mode);
        if (!VirementSalaireBillingService.MODES.contains(resolvedMode))
            return Response.status(400).entity(ApiError.of("BAD_REQUEST",
                    "Mode de facturation invalide (NONE | FLAT | STANDARD)", path)).build();

        String country = getCachedUserCountry(batch.uploadedById);
        var billing = billingService.computeForBatch(bId, resolvedMode,
                billingService.resolveFlatFee(flatFee), country);
        return Response.ok(billing.toMap()).build();
    }

    /**
     * Fixe le mode de facturation choisi pour ce lot (avant validation/traitement).
     * Renvoie l'aperçu final (Z, X, frais, A) effectivement enregistré.
     */
    @POST
    @Path("/{id}/billing")
    @RolesAllowed({"INPUTTER", "ADMIN"})
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Operation(summary = "Définir les charges d'un lot de virements de salaires")
    public Response setBilling(@PathParam("id") String id, BillingRequest request) {
        String path = "/api/v1/batches/" + id + "/billing";
        ObjectId bId = parseObjectId(id);
        FileBatch batch = FileBatch.findById(bId);
        if (batch == null)
        ...
        // Ownership (non-admin must be the uploader)
        if (!identity.hasRole("ADMIN")) {
            String me = identity.getPrincipal().getName();
            if (!me.equals(batch.uploadedById))
                return Response.status(403).entity(ApiError.of("FORBIDDEN", "Accès refusé", path)).build();
        }

        // Only before the batch leaves the operator's hands (i.e. before validation/processing)
        if (!FileBatch.STATUS_UPLOADED.equals(batch.status))
            return Response.status(409).entity(ApiError.of("CONFLICT",
                    "Les charges doivent être fixées tant que le lot est UPLOADED (actuel: " + batch.status + ")", path)).build();

        String resolvedMode = billingService.resolveMode(request == null ? null : request.billingMode());
        if (!VirementSalaireBillingService.MODES.contains(resolvedMode))
            return Response.status(400).entity(ApiError.of("BAD_REQUEST",
                    "Mode de facturation invalide (NONE | FLAT | STANDARD)", path)).build();

        BigDecimal flatFee = request == null ? null : request.flatFeeAmount();
        if ("FLAT".equals(resolvedMode)) {
            if (flatFee == null || flatFee.signum() <= 0)
                return Response.status(400).entity(ApiError.of("BAD_REQUEST",
                        "Le mode FLAT exige un frais forfaitaire strictement positif", path)).build();
        } else {
            flatFee = null; // non pertinent hors FLAT
        }

        batch.billingMode = resolvedMode;
        batch.flatFeeAmount = flatFee;
        batch.update();

        AdminAuditLog.record(identity.getPrincipal().getName(), "BATCH_BILLING_SET", id,
                "Charges définies (" + resolvedMode + ") pour " + batch.originalFilename,
                Map.of("mode", resolvedMode, "flatFee", String.valueOf(flatFee)));

        String country = getCachedUserCountry(batch.uploadedById);
        var billing = billingService.computeForBatch(bId, resolvedMode, flatFee, country);

        Map<String, Object> body = new LinkedHashMap<>();
        body.put("message", "Charges enregistrées");
        body.put("batchId", id);
        body.put("billing", billing.toMap());
        return Response.ok(body).build();
    }

    public record BillingRequest(
            @JsonProperty("billingMode") String billingMode,
            @JsonProperty("flatFeeAmount") BigDecimal flatFeeAmount) {
    }
